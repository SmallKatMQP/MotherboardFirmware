...
void Leg::AutoAssignID() {
	...
	HAL_SPI_Transmit(hspi, (uint8_t*)TransData, 29, 290);
...
void Leg::updateLeg(uint16_t* Motor1, uint16_t* Motor2, uint16_t* Motor3,
	...
	*TransData = {0};
	...
	//	}
	TransData[0][0] = Motor1[0];
	TransData[0][1] = Motor1[1];
	TransData[0][2] = Motor1[2];
	TransData[0][3] = Motor1[3];
	TransData[1][0] = Motor2[0];
	TransData[1][1] = Motor2[1];
	TransData[1][2] = Motor2[2];
	TransData[1][3] = Motor2[3];
	TransData[2][0] = Motor3[0];
	TransData[2][1] = Motor3[1];
	TransData[2][2] = Motor3[2];
	TransData[2][3] = Motor3[3];
	TransData[3][0] = Motor4[0];
	TransData[3][1] = Motor4[1];
	TransData[3][2] = Motor4[2];
	TransData[3][3] = Motor4[3];

	for(int i = 0; i<4;i++)
		GPIO->ODR &= ~GPIO_Pin; //set cs pin low
		HAL_SPI_TransmitReceive_DMA(hspi, (uint8_t*)TransData, (uint8_t*)retData,  5);//send setpints and get prev data
		GPIO->ODR |= GPIO_Pin; //set cs pin high
}
